...
typedef struct hashtable {
    ...
    Continue_Memroy* (*handleKeyValuePtr)(void* key, void* value_ptr, Action_On_Key_Value action_on_key_value, struct hashtable* hashtable);
...
HashTable* setupHashTable(unsigned int initial_size, unsigned int max_ht_size_limit, size_t key_size, Continue_Memroy* (*handleKeyValuePtr) (void* key, void* value_ptr, Action_On_Key_Value action_on_key_value, HashTable* hashtable), Pair* generateKeyAndValuePtr(size_t key_size, FILE* file_ptr), char* filepath, uint64_t (*hashFunction) (void* value, size_t size), CollisionHandling collision_handling);
